let canvas = document.getElementById('game'),
    ...
    ballRadius = 9,
    x = canvas.clientWidth / (Math.floor(Math.random() * Math.random() * 10) + 3),
    y = canvas.clientHeight - 40,
    dx = 2,
    dy = -2;

let paddleHeight = canvas.clientHeight / 20,
    paddleWidth = canvas.clientWidth / 12;

// Paddle start position
let paddleX = (canvas.clientWidth - paddleWidth) / 2;
...
function drawPaddle() {
    ctx.beginPath();
    ctx.roundRect(paddleX, canvas.clientHeight - paddleHeight, paddleWidth, paddleHeight, 30);
